...
def trou(canvas, xD,yD,echelle=1,largeur=-1, sens = HORIZONTAL, **kwargs ):
    ...
    canvas.create_rectangle(xD+space//3, yD+space//3, xD+2*space//3,yD+2*space//3,fill="#484848",outline="#484848")
...
pe630pts = {"lgLigne":66, "lgColonne":22, "sepAlim":[4,18.5], "sepDistribution":[10.7]}
...
def planche(canvas,xD=0,yD=0,echelle=1,largeur=-1, sens = VERTICAL, **kwargs ):
    ...
    rectangleArrondi(canvas, xD, yD, dimLigne,  dimColone, 20, outline="#F5F5DC", fill="#F5F5DC", width=epaisseur)
    for sepY in sepAlim:
        canvas.create_line(xD , yD+interSpace*sepY, xD + dimLigne, yD + interSpace*sepY, fill="#707070", width=epaisseur) 
    for sepY in sepDistribution:      
        canvas.create_line(xD , yD+interSpace*sepY, xD + dimLigne, yD + interSpace*sepY, fill="#e5e5cC", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY + epaisseur, xD + dimLigne, yD + interSpace*sepY + epaisseur, fill="#d5d5bC", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY + 2*epaisseur, xD + dimLigne, yD + interSpace*sepY  + 2*epaisseur, fill="#c5c5aC", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY + 3*epaisseur, xD + dimLigne, yD + interSpace*sepY  + 3*epaisseur, fill="#b0b097", width=epaisseur)    
        for dy in range(4,11):
            canvas.create_line(xD , yD+interSpace*sepY + dy*epaisseur, xD + dimLigne, yD + interSpace*sepY + dy*epaisseur, fill="#eaead1", width=epaisseur)   
        canvas.create_line(xD , yD+interSpace*sepY + interSpace - 4*epaisseur, xD + dimLigne, yD + interSpace*sepY+interSpace - 4*epaisseur, fill="#b0b097", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY + interSpace - 3*epaisseur, xD + dimLigne, yD + interSpace*sepY + interSpace - 3*epaisseur, fill="#c5c5a5", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY+ interSpace - 2*epaisseur, xD + dimLigne, yD + interSpace*sepY  + interSpace - 2*epaisseur, fill="#d5d5bC", width=epaisseur)    
        canvas.create_line(xD , yD+interSpace*sepY + interSpace - epaisseur, xD + dimLigne, yD + interSpace*sepY + interSpace - epaisseur, fill="#e5e5cC", width=epaisseur)    
...
def remplirMatrice630pts(colD=1,  ligneD = 1, **kwargs):
    interSpace = 15
...
def remplirMatrice1260pts(colD=1,  ligneD = 1, **kwargs):
    ...
    remplirMatrice630pts(matrice=matrice1260pts)
    remplirMatrice630pts(ligneD=15, matrice=matrice1260pts)
...
plaquette630pts = [(setXYOrigine,1,{"idOrigine":"plq630"}),(planche,1),(demiPlat,1,{"sens":HORIZONTAL}),(demiPlat,1,{"sens":VERTICAL}),(bandeAlim,1,{"sens":VERTICAL}), \
                   (numerotation,1,{"sens":VERTICAL}), (goXY,1,{"ligne":5.5, "colonne":0.5, "idOrigine":"plq630"}), \
                   (carIter,1,{"carDeb":"f", "nbCar":5, "anchor":"center", "deltaY":0.7}),(bandeDistribution,1,{"sens":VERTICAL}),(goXY,1,{"ligne":5.5, "colonne":64.5, "idOrigine":"plq630"}),(demiPlat,1),(carIter,1,{"carDeb":"f", "nbCar":5, "sens":VERTICAL, "deltaY":0.7}), \
                   (goXY,1,{"ligne":12.5, "colonne":0.5, "idOrigine":"plq630"}), \
                   (carIter,1,{"carDeb":"a", "nbCar":5, "deltaY":0.7}), (bandeDistribution,1,{"sens":VERTICAL}), (goXY,1,{"ligne":12.5, "colonne":64.5, "idOrigine":"plq630"}),(demiPlat,1),(carIter,1,{"carDeb":"a", "nbCar":5, "sens":VERTICAL, "deltaY":0.7}), \
                   (goXY,1,{"ligne":18.8, "colonne":0.5, "idOrigine":"plq630"}), (numerotation,1,{"sens":VERTICAL}),
                   (goXY,1,{"ligne":18.5, "colonne":0.5, "idOrigine":"plq630"}), (bandeAlim,1,{"sens":VERTICAL})]    # ,1,{"sens":VERTICAL})]

plaqueTest = [(setXYOrigine,1),(bandeAlim,1,{"sens":HORIZONTAL})]
plaqueTestMinimal = [(setXYOrigine,1),(railAlimPlus,4,{"sens":VERTICAL})]
plaquette1260pts =[(plaquette630pts,2,{"sens":PERSO, "dXY": (0, 1.3)})]
    ...
    for element in modele:
        if callable(element[0]) and isinstance(element[1],int):
            for _ in range(element[1]):
                if len(element) == 3:
                    (x, y) = element[0](canvas, x, y, echelle, largeur,**element[2])
                else : (x, y) = element[0](canvas, x, y, echelle, largeur)
        elif isinstance(element[0],list) and isinstance(element[1],int) :
            for _ in range(element[1]):
                if len(element) == 3:
                    (x,y) = circuit(canvas,x,y,echelle,largeur, modele=element[0], **element[2] )
                else : (x,y) = circuit(canvas,x,y,echelle,largeur,modele=element[0])
